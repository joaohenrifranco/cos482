<template>
  <div>
    <h2 id="page-heading" data-cy="ProcessInstanceHeading">
      <span v-text="$t('ingressaApp.processInstance.home.title')" id="process-instance-heading">Process Instances</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('ingressaApp.processInstance.home.refreshListLabel')">Refresh List</span>
        </button>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && processInstances && processInstances.length === 0">
      <span v-text="$t('ingressaApp.processInstance.home.notFound')">No processInstances found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="processInstances">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('ingressaApp.processInstance.businessKey')">Business Key</span></th>
            ...
            <th scope="row"><span v-text="$t('ingressaApp.processInstance.camundaDeploymentId')">Camunda Deployment Id</span></th>
            <th scope="row">
              <span v-text="$t('ingressaApp.processInstance.camundaProcessDefinitionId')">Camunda Process Definition Id</span>
            </th>
            <th scope="row">
              <span v-text="$t('ingressaApp.processInstance.camundaProcessInstanceId')">Camunda Process Instance Id</span>
            </th>
            <th scope="row"><span v-text="$t('ingressaApp.processInstance.camundaProcessVariables')">Camunda Process Variables</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.processInstance.processDefinition')">Process Definition</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="ingressaApp.processInstance.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-processInstance-heading" v-text="$t('ingressaApp.processInstance.delete.question', { id: removeId })">
