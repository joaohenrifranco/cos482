<template>
  <div>
    <h2 id="page-heading" data-cy="TaskInstanceHeading">
      <span v-text="$t('ingressaApp.taskInstance.home.title')" id="task-instance-heading">Task Instances</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('ingressaApp.taskInstance.home.refreshListLabel')">Refresh List</span>
        </button>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && taskInstances && taskInstances.length === 0">
      <span v-text="$t('ingressaApp.taskInstance.home.notFound')">No taskInstances found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="taskInstances">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.taskId')">Task Id</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.name')">Name</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.status')">Status</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.createDate')">Create Date</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.createTime')">Create Time</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.dueDate')">Due Date</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.startTime')">Start Time</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.endTime')">End Time</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.owner')">Owner</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.assignee')">Assignee</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.executionId')">Execution Id</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.taskDefinitionKey')">Task Definition Key</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.suspended')">Suspended</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.priority')">Priority</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.processDefinition')">Process Definition</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.taskInstance.processInstance')">Process Instance</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
