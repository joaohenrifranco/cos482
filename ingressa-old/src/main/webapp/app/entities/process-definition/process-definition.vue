<template>
  <div>
    <h2 id="page-heading" data-cy="ProcessDefinitionHeading">
      <span v-text="$t('ingressaApp.processDefinition.home.title')" id="process-definition-heading">Process Definitions</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('ingressaApp.processDefinition.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link
        ...
        >
          ...
          <span v-text="$t('ingressaApp.processDefinition.home.createLabel')"> Create a new Process Definition </span>
        </router-link>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && processDefinitions && processDefinitions.length === 0">
      <span v-text="$t('ingressaApp.processDefinition.home.notFound')">No processDefinitions found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="processDefinitions">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('ingressaApp.processDefinition.name')">Name</span></th>
            <th scope="row"><span v-text="$t('ingressaApp.processDefinition.camundaDeploymentId')">Camunda Deployment Id</span></th>
            <th scope="row">
              <span v-text="$t('ingressaApp.processDefinition.camundaProcessDefinitionId')">Camunda Process Definition Id</span>
            </th>
            <th scope="row">
              <span v-text="$t('ingressaApp.processDefinition.bpmnProcessDefinitionId')">Bpmn Process Definition Id</span>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="ingressaApp.processDefinition.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-processDefinition-heading" v-html="$t('ingressaApp.processDefinition.delete.question', { id: removeId })">
